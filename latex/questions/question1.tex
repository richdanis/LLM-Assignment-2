...
As there are $2$ adapters and $2$ layer norms per Transformer layer, we have $2N (2  m d + m + d) + 2 N \times 2d = 4 N m d + 2 N m + 6 N d$ trainable parameters in total.
...
Finally, for all $N$ layers and $H$ attention heads per layer, we thus have $N (3 H (d r + r d_h) + 2 dr)$ parameters in total.
}
\end{subquestion}
...
Further we have to account for the parameters of the smaller matrix, which is of size $l \times d_h$.
That makes in total $N d' (d_h + d) + l d_h$ trainable parameters.
}
\end{subquestion}
