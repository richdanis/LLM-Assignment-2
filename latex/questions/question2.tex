...
\begin{subquestion}{~\small \textbf{(4 pts)}}
    ...
    \solution{
        ...
        To reach optimality, we could fix $D$ and then decrease $N$ till the equation $(ND)^{0.46} = N$ is satisfied.
        Precisely we would need to decrease $N$ to,
        \begin{equation*}
            \begin{split}
                (ND)^{0.46} &= N \\
                D^{0.46} &= N^{0.54} \\
                N &= D^{\frac{0.46}{0.54}} = 5.98 * 10^9.
            \end{split}
        \end{equation*}
    }
\end{subquestion}
\begin{subquestion}{~\small \textbf{(3 pts)}}
    ...
    \solution{
        ...
        The given power laws also imply that the loss will get very small for large $N$ and $D$.
        ...
        Further the loss depends on training time as well, which naturally grows with bigger datasets and more parameters.
        But this quantity is not considered in the given power laws.
    }
