...
\begin{subquestion}{~\small \textbf{(4 pts)}}
    ...
    \solution{
        ...
        \begin{equation*}
            \begin{split}
                C^{0.46} &= (ND)^{0.46} = 2.19 * 10^{10} \neq N = 100 * 10^9 = 10^{11} \\
                C^{0.54} &= (ND)^{0.54} = 1.37 * 10^{12} \neq D = 300 * 10^9 = 3 * 10^{11}.
            \end{split}
        \end{equation*}
        Since the numbers do not match, we can conclude that the model is not trained optimally.
        To reach optimality, we could fix $D$ and then increase $N$ till the equation $(ND)^{0.46} = N$ is satisfied.
        Precisely we would need to increase $N$ to,
        \begin{equation*}
            \begin{split}
                (ND)^{0.46} &= N \\
                D^{0.46} &= N^{0.54} \\
                N &= D^{\frac{0.46}{0.54}} = 128.9 * 10^9.
            \end{split}
        \end{equation*}
    }
\end{subquestion}
